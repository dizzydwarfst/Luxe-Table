...
const PreviewScreen: React.FC<Props> = ({ item, menu, onBack, onAR, onAddToCart }) => {
  ...
  const useThreeJS = !!item.modelUrl; // use Three.js viewer for items with 3D models
    ...
    <div className="flex flex-col h-full bg-slate-900 text-white relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex-1 relative w-full" style={{ minHeight: 0 }}>
          ...
          {useThreeJS ? (
            // Three.js — pizza + toppings in ONE scene, rotate together!
            <PizzaViewer
              pizzaUrl={item.modelUrl!}
              selectedToppings={selectedToppings}
            />
          ) : (
            // Fallback: model-viewer for items without topping support
            ...
            <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4">
                ...
                {item.availableToppings!.map(topping => {
                  ...
                  return (
                    <button
                      ...
                      className={`
                        relative flex items-center gap-2 px-4 py-2.5 rounded-xl border-2 font-bold text-xs
                    ...
                    >
                      ...
                      <span className={`
                        ...
                        ${isSelected
                          ...
                          : 'bg-slate-800 text-white/20 border-white/10'
                        }
                      `}>
              ...
              {selectedToppings.length > 0 && (
                <p className="text-[9px] text-white/25 mt-3">
                  {selectedToppings.length} topping{selectedToppings.length > 1 ? 's' : ''} · rotating with the pizza in 3D
                </p>
              )}
            </div>
          </div>
        ...
        <div className="px-5 mb-4">
          <div className="bg-navy/80 backdrop-blur-md border border-primary/20 rounded-2xl p-4">
